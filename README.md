...
1. 下载脚本:
    ```shell
    wget --no-check-certificate https://raw.githubusercontent.com/jethan/vpn/master/ikev2.sh
    wget --no-check-certificate https://raw.githubusercontent.com/jethan/vpn/master/l2tp.sh
    ```
...
5. 是否使用SNAT规则(可选).默认为不使用.使用前请确保服务器具有不变的**静态公网ip**,可提升防火墙对数据包的处理速度.如果服务器网络设置了NAT(如AWS的弹性ip机制),则填写网卡连接接口的ip地址(参见[KinonC](https://github.com/KinonC).
 ...
 * **iOS/OSX** 的远程ID和服务器地址保持一致,用户鉴定选择"用户名".如果通过浏览器导入,将证书放在可访问的远程外链上,并在**系统浏览器**(Safari)中访问外链地址.OSX证书需要设置为始终信任;
...
2. 删除脚本所在目录的相关文件(ikev2.sh,strongswan.tar.gz,strongswan文件夹,my_key文件夹).
